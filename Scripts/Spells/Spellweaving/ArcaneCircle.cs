	...
	public class ArcaneCircleSpell : ArcanistSpell
	{
		...
		public override void OnCast()
		{
			if( CheckSequence() )
			{
				...
				TimeSpan duration = TimeSpan.FromHours( Math.Max( 1, (int)(Caster.Skills.Magery.Value / 24) ) );
		...
		private List<Mobile> GetArcanists()
		{
			...
			{
				if ( m != Caster && Caster.CanBeBeneficial( m, false ) && Math.Abs( Caster.Skills.Magery.Value - m.Skills.Magery.Value ) <= 20 && !(m is Clone) )
				{
					weavers.Add( m );
				}
