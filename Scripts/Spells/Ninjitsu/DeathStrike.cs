...
namespace Server.Spells.Ninjitsu
{
	public class DeathStrike : NinjaMove
	{
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			double ninjitsu = attacker.Skills[SkillName.Magery].Value;
			...
			if( m_Table.Contains( defender ) )
			{
				...
				if( info.m_Steps > 0 )
					damageBonus = attacker.Skills[SkillName.Magery].Fixed / 150;
		...
		private static void ProcessDeathStrike( object state )
		{
			...
			double ninjitsu = info.m_Attacker.Skills[SkillName.Magery].Fixed;
