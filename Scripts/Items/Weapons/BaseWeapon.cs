...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
	{
        ...
        private ZuluCombat.DiceRoll m_DiceRoll;       
        private int m_HitscriptData;
        private int m_HitscriptChance;
        private ZuluHitscript m_Hitscript;
        ...
        public virtual string DiceExpression
        {
            get
            {
                return String.Format("1d{0}", this.OldMaxDamage);
            }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual ZuluCombat.DiceRoll DiceRoll
        {
            get 
            {
                if (m_DiceRoll == null)
                {
                    m_DiceRoll = new ZuluCombat.DiceRoll(this.DiceExpression);
                    m_DiceRoll.Modifier += ZuluCombat.GetDamageLevel(this.DamageLevel);
                }

                return m_DiceRoll;
            }
            set { m_DiceRoll = value; }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual int HitscriptChance
        {
            get { return m_HitscriptChance; }
            set { m_HitscriptChance = (value > 100) ? 100 : value; }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual int HitscriptData
        {
            get { return m_HitscriptData; }
            set { m_HitscriptData = value; }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual ZuluHitscript Hitscript
        {
            get { return m_Hitscript; }
            set { m_Hitscript = value; }
        }
		...
		public WeaponDamageLevel DamageLevel
		{
			...
			set
            {
                DiceRoll.Modifier -= ZuluCombat.GetDamageLevel(m_DamageLevel);
                DiceRoll.Modifier += ZuluCombat.GetDamageLevel(value);
			...
			if ( defender.Player || defender.Body.IsHuman )
			{
				...
				if ( blocked )
				{
					...
					/*if ( Confidence.IsConfident( defender ) )
					{
						...
						double bushido = defender.Skills.Magery.Value;
		...
		{
            //Remove weapon ability -- jabs
			/*if ( MirrorImage.HasClone( defender ) && (defender.Skills.Magery.Value / 150.0) > Utility.RandomDouble() )
			{
				Clone bc;
            ...
            int damage = ZuluCombat.CombatHit(attacker, defender, this, true);
		...
		public virtual void GetStatusDamage( Mobile from, out int min, out int max )
		{
            ...
            min = DiceRoll.GetMinimumValue();
            max = DiceRoll.GetMaximumValue();
