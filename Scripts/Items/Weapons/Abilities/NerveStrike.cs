...
namespace Server.Items
{
	...
	public class NerveStrike : WeaponAbility
	{
		...
		{
			if( GetSkill( from, SkillName.Magery ) < 50.0 )
			{
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			if ( Core.ML )
			{
				AOS.Damage( defender, attacker, (int)( 15.0 * ( attacker.Skills[SkillName.Magery].Value - 50.0 ) / 70.0 + Utility.Random( 10 ) ), true, 100, 0, 0, 0, 0 );	//0-25
				if ( ( ( 150.0 / 7.0 + ( 4.0 * attacker.Skills[SkillName.Magery].Value ) / 7.0) / 100.0 ) > Utility.RandomDouble() )
				{
			...
			{
				AOS.Damage( defender, attacker, (int)(15.0 * (attacker.Skills[SkillName.Magery].Value - 50.0) / 70.0 + 10), true, 100, 0, 0, 0, 0 ); //10-25
