...
namespace Server.Items
{
	...
	public class Feint : WeaponAbility
	{
		...
		{
			if( GetSkill( from, SkillName.Magery ) < 50.0  && GetSkill( from, SkillName.Magery ) < 50.0 )
			{
		...
		public override void OnHit( Mobile attacker, Mobile defender, int damage )
		{
			...
			Timer t = new FeintTimer( defender, (int)(20.0 + 3.0 * (Math.Max( attacker.Skills[SkillName.Magery].Value, attacker.Skills[SkillName.Magery].Value ) - 50.0) / 7.0) );	//20-50 % decrease
