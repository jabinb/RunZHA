...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
        ...
        public override bool HandlesOnSpeech(Mobile from)
        {
            ...
            return (from is PlayerMobile) && from.InRange(this.Location, 1);
        }

        public override void OnSpeech(SpeechEventArgs e)
        {
            if (!(e.Mobile is PlayerMobile) || !e.Mobile.InRange(this.Location, 1))
                return;

            string input = e.Speech.Trim().ToLowerInvariant();

            switch (input)
            {
                case "buy": VendorBuy(e.Mobile); break;
                case "sell": VendorSell(e.Mobile); break;
                default:
                    break;
            }
