	...
	public class RegenRates
	{
		...
		private static TimeSpan Mobile_HitsRegenRate( Mobile from )
		{
			...
			/*if ( CheckAnimal( from, typeof( Dog ) ) || CheckAnimal( from, typeof( Cat ) ) )
				points += from.Skills[SkillName.Magery].Fixed / 300;*/
		...
		private static TimeSpan Mobile_StamRegenRate( Mobile from )
		{
			...
			CheckBonusSkill( from, from.Stam, from.StamMax, SkillName.Magery );

			int points =(int)(from.Skills[SkillName.Magery].Value * 0.1);
		...
		private static TimeSpan Mobile_ManaRegenRate( Mobile from )
		{
			...
			if ( Core.AOS )
			{
				...
				CheckBonusSkill( from, from.Mana, from.ManaMax, SkillName.Magery );

				double focusPoints = (from.Skills[SkillName.Magery].Value * 0.05);
