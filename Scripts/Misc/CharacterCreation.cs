...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void SetAllSkillcaps( Mobile from, double cap ) //na85
		{
			for ( int i = 0; i < SkillInfo.Table.Length; i++ )
			{
				from.Skills[i].Cap = cap;
			}
        }
		...
		private static void SetSkills( Mobile m, SkillNameValue[] skills, int prof )
		{
			switch ( prof )
			{
				...
				case 5: // Paladin
				{
					skills = new SkillNameValue[]
						{
							new SkillNameValue( SkillName.Healing, 51 ),
							new SkillNameValue( SkillName.Swords, 49 ),
							new SkillNameValue( SkillName.Macing, 30 ),
				...
				case 7:	//Ninja
				{
					skills = new SkillNameValue[]
						{
							new SkillNameValue( SkillName.MagicResist, 50 ),
