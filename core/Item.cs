...
namespace Server
{
	...
	{
        public static readonly Ultima.ClilocList ClilocList = new Ultima.ClilocList("enu");
		...
		{
			return StackWith( from, dropped, false );
		}

		public virtual bool StackWith( Mobile from, Item dropped, bool playSound )
		{
			if ( dropped.Stackable && Stackable && dropped.GetType() == GetType() && dropped.ItemID == ItemID && dropped.Hue == Hue && dropped.Name == Name && (dropped.Amount + Amount) <= MaximumAmount )
			{
        ...
        public virtual int MaximumAmount
        {
            get { return 60000; }
        }
		...
		public virtual bool DisplayLootType{ get{ return false; } }
		...
		public virtual void OnSingleClick( Mobile from )
		{
			...
			if ( ns != null )
			{
                ...
                if ( this.Name == null && ClilocList.Table.Contains( LabelNumber ) )
				{
                    /*
					if ( m_Amount <= 1 )
                    ...
                    */
                    ...
                    bool Plural = m_Amount > 1;

                    if(Plural)
                        LabelTo( from, String.Format( "{0} {1}s", m_Amount, ClilocList.Table[LabelNumber] ) );
                    else
                        LabelTo( from, (string)ClilocList.Table[LabelNumber] );
				}
				else
				{
                    ...
                    LabelTo(from, this.Name + (m_Amount > 1 ? " : " + m_Amount : ""));
                }
			}
		}
